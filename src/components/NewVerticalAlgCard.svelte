...
<div id={setName + "-" + caso.name} class="w-[95%] m-4 ml-0 mt-0 pt-4 flex flex-row items-center justify-center">
  ...
  bg-stone-900/[.07] dark:bg-stone-50/[.07] shadow-lg hover:shadow-2xl hover:-translate-y-[1px]">
    <div class="absolute top-0 right-0 w-full flex justify-between items-center gap-1 px-2 pb-1 translate-y-[-1.5em]
    text-[3.2vw] sm:text-[20px]">
      <div>
        <a href="#{setName}-{caso.name}" class="">
          <b>{caso.name}</b>
        </a>
        
      </div>
      <span class="my-[2px] px-2 py-[1px] text-sm bg-stone-50/[.15] rounded-lg shadow-md cursor-pointer
    ...
    <div class="flex items-center gap-1 relative w-full leading-none overflow-x-visible">
      ...
      on:click={() => {  }} title="Expand Alg">⟰</span> -->
      <!-- TODO: add dialog with the expanded alg on hover, also process algorithm so you can style each trigger individually -->
      <span class="{activeElement == i ? 'text-[2.9vw] sm:text-[18.5px] font-bold text-stone-600 dark:text-stone-400' : 'text-[2.8vw] sm:text-[17.5px] text-stone-400 dark:text-stone-500'}
      hover:text-[3.0vw] sm:hover:text-[19.5px] hover:text-stone-900 dark:hover:text-stone-100 transition-all cursor-pointer
      ...
      <span class="flex m-1 justify-end gap-1 cursor-default text-stone-500">
        {#if alg.tags}Tags:{#each alg.tags as tag}
