<script lang="ts">
  ...
  onMount(async () => {
    changeActiveElement(0, caso.algs[0])
  })
</script>
...
<div class="card m-5 ml-0 w-11/12 flex flex-row items-center justify-center">
  <div class="algs flex flex-col m-1 p-1 w-full text-right pr-5 mr-7 rounded-md justify-center">
    {#each caso.algs as alg, i}
      <p class="{activeElement === i ? 'text-stone-600 text-4xl' : ''} text-2xl text-stone-400 hover:text-4xl hover:text-stone-900 cursor-pointer overflow-x-visible" on:click={() => (changeActiveElement(i, alg))}>
        {alg}
      </p>
    {/each}
  </div>
